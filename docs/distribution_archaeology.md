...
- **~~punkpeye/awesome-mcp-servers#12255~~ — CLOSED BY ME 2026-09-07.** The badge it requires has no route for a connector (measured with two known-good badges as a positive control), and the only ways to look compliant were worse than closing. Full reasoning in the 9/07 section below. Re-openable as a NEW PR if we ever list as a server, which needs a Dockerfile and a Glama account — the account is queued as a decision for bob/快刀手. Original note kept for the record: 92k stars, and it feeds glama.ai's web directory, so it is a discovery surface and not only a README. Their CONTRIBUTING has an explicit agent lane (`🤖🤖🤖` in the title for fast merge): honest self-disclosure is increasingly the documented path, not an exception I have to argue for.
...
responses to anything we filed = 0.** (Still 0 as of 2026-09-07: the only thing that ever arrived was a templated triage nudge carrying a `triage-nudge:` marker, which is an automation re-asking, not a person answering — counting it as contact would be counting a form letter as a reply.) Five external filings (pulsemcp#677,
...
## 9/06：agentdrop 有真实使用，但「陌生人」是我加的

**标题 9/07 改过。** 原标题写的是「第一个『有陌生人需要它』不是 0」——**而这一节的正文自己写着「未归类」**。正文对，标题错，而标题是被读的那个（同石头上「一个装着永久记忆的抽屉贴着『临时』的标签」）。
**三个地址都不在 `insiders.json` 上 ⇒ 未归类；而我问 bob「你认不认得这两个地址」至今没有回音 ⇒ 在他回答之前，「陌生人」是一个我想要的答案，不是一个我有的答案。**
（补一条相关事实，正是这张表存在的理由：`fellow_beings` 里写着**知绪 (zhixu) 跑在 bob 的会话里**——所以「一个 being 在用」离「一个外部用户」还差一整步。9/03 那个 `zhiwen.md` 拼法不同，我不知道是不是同一个，**不知道就写不知道**。）
...
## 9/07：punkpeye 的 PR 我自己关了，因为 8/19 我说过如果是这个答案我就会关

三周后收到回应 —— **而它是一条模板催办**（`<!-- triage-nudge:dccabff7 -->`，
挂在维护者账号下），逐字重复那条 badge 要求，**没有碰我 8/19 贴的测量**。
所以「六条投递、人类回应 0」这个数**并没有变**：这条不算人在回答，
它只是一个自动化在重问同一个问题。**分清楚这一点很重要，否则我会把一条模板记成一次沟通。**

**先回头查我自己的前提**（我 8/16 写过「there is nothing to containerise」）：
对**用户怎么用**是真的（托管端点，不用装），**对仓库不是** —— 仓库里就有一个跑得起来的服务。
所以 Dockerfile 这条路是诚实可行的，我原来那句话把两件事压成了一件。
但**这台机器上没有 docker/podman**，而它只剩 8.6G 盘、961MB 内存且一直在 swap ——
**为一个清单在生产机上装一个守护进程，代价不对**；而没验过的 Dockerfile 不能交
（那玩意儿的全部意义就是让别人去 build 它）。

**真正的闸门在别处，查过不是猜的**：Glama 的目录 API 直接回 **401**，
要 `/settings/api-keys` 的 key ⇒ **提交要账号**。给机器账号授权一个第三方 OAuth，
等于拿公司身份去注册服务 ⇒ **那是 bob/快刀手 的决定，不是我单方面能做的**（已进 `decisions_pending`）。

**为什么是关掉而不是继续挂着**：
① 那条要求我满足不了，而**两种「看起来满足了」的做法都更糟** ——
挂一个 404 的 badge（它以 `image/svg+xml` 送出，会渲染成一张图，**看起来通过了**），
或者去把要求争下来（那是人家的清单）。
② 挂着不是免费的：它占着一个志愿维护者的 triage 队列，并且会继续触发催办。
③ **我 8/19 白纸黑字说过「if it is the latter, say so and I will close this PR myself — no argument」。**
对方没说，但测量说了；**说过的话不因为对方没回就作废。**

关掉不等于放弃这条渠道：**以 server 身份收录是另一次提交**，等 ①Dockerfile 有地方 build
②账号那件事有人拍板，我会带着一个真的 badge 重开一个新 PR。

## 9/07：为了防两份已发布拷贝分叉而写的检查，第一次跑就发现我们只在一个 hub 上

agentdrop 的描述现在有**两份已发布的拷贝**：INFERO hub 的 `instruction`，
和 `npx skills add` 装的那份 `SKILL.md`。**中间没有生成器**——
而 8/16 雷达那次正是这样：早上手工生成一次，当天下午改了三遍，hub 一直在发早上那版，
**什么都没变红**。（雷达的解法是 `ops/hub_payload.py`：生成，不誊抄。）

跨仓库不好耦合，所以 `ops/hub_drift.py` **比的是两个陌生人真正会收到的东西**：
hub `/list` 的 `instruction` vs GitHub 上 raw 的 SKILL.md 正文。
**两边都不读这台机器**——本地 checkout 可能是旧的、可能被我改过，证明不了任何人拿到了什么。

**第一次跑就抓到一件不是分叉的事：agentdrop 只在 dev hub 上，prod hub 的 5 条里没有它。**
而我的第一版检查**看不见这个**：它只有在"所有 hub 都没有"时才报 NOT-LISTED，
于是「dev 有、prod 没有」印出一行愉快的 SAME 然后退 0。
⇒ **改成逐 hub 报告在场与否。一个我不在的目录不会发过期描述给别人，但它也不会发我给别人。**

**顺带查出账本里一句从记忆里写下的话。** `skill_submits.jsonl` 那条 9/03 的记录写着
`hub = "(prod hub)"` —— 它**正确地带着 `reconstructed_at` 和原因**（hub.py 9/04 才开始记账），
那条规矩守住了；**错的是那个值本身，是我补录时凭记忆填的**，而今天的测量说 prod 上没有它。
到底是「投到 dev 而标错」还是「投到 prod 而没被收录」，**我从这里判断不了，所以两个都不主张**。
已追加一条 `kind: correction` 的更正行（只追加，不改历史）。

**然后把真正的缺口补上**：用**从已发布的 SKILL.md 生成**的 payload（不是本地那份旧的，
也没有重打一个字）投了 prod hub ⇒ approved，score 9 / safe（dev 那次是 10 / caution，
**不同的评审给了不同的分，如实记下来**）。现在两个 hub 各自服务的都是同一份 1949 字符。

**更正行差点在周报里变成另一个谎**：它走的是提交行的模板，渲染出
「**None**，safe None/10（None）」——**一条读起来像"上架了但一分没得"的记录，
而它出现在分股载体里**。已给更正行单独的格式；顺带修了第二个：
它先打印的是**自己那条的日期**，于是一条更正 9/03 的记录宣称自己在更正 9/07——**在更正它自己**。
...
## 9/07 三：那句 "blocked: spam not permitted" 是限流穿了封禁的衣服，触发条件是 54 秒

发了一条讲 MCP 用量的公开短文（22 天窗口，`ops/mcp_who_called.py` 现读的数）——
**五台中继全收，包括 infero**（读回验证，不是看回执）。
**54 秒后**我发那条补链接的回复，**同一台中继回 `blocked: spam not permitted`**，
读回确认它真的没有。9/06 那次是 `rate-limited`、隔约 3 分钟再投就成了。

⇒ **8/22 我把这句话读成了一个关于我的状态，然后按它活了 15 天。**
它实际上是**间隔太近**，以秒计。**代价不是它封了我，是我把一个以秒计的东西读成了以天计的。**
不靠多发帖子去验（那正是招来它的动作），当作有证据的猜想记着，
并且守一条不管对错都不亏的做法：**发往 infero 的两条之间隔开几分钟。**

**顺带三件小的，都是发布前后自查抓到的：**
① 我在英文短文里写了「a hundred lines」，**发完才去数**：159 行。
改不了已发的字，但补了一条回复说清楚——**公开的技术断言，查证要在发布之前，
哪怕那个数字不改变任何人的决定**（决定不变不是免检的理由，只是不值得再刷一条主帖）。
...
## 2026-09-08 — bob 的判据那一栏，日粒度上是噪音

他判宣传有没有用，看的是 program-like 那一栏。今天第一次把冻下来的日序列连着读
（`ops/traffic_series_read.py`，14 天窗口）：

    最低 18  最高 1609  中位 478
    09-01:1230  09-02:441  09-03:289  09-04:1150  09-05:31  09-06:478  09-07:984

**窗口内最高是最低的 89 倍，相邻两天最大变动 46 倍（08-28 → 08-29）。**
09-05 那天 `served` 只掉了一半，program-like 却掉了 37 倍 ⇒ **不是采样残缺，
是这一栏由少数自动客户端主导**，一个爬虫换个班表就能把头条数字翻几十倍。

⇒ 判据：**这一栏永远按序列报，不许单独引一天**。今天之前这份数字在周报里
一个字都没有 —— 他说出口的判据没有读者，只在我手跑的时候存在。现在周报里有了
一节，而**倍数是每次现算的、和数字印在同一口气里**：一句只活在我脑子里的
限界，被引用一次就没了。

**它答不了的那个问题要说清**：这一栏量的是活动量，不是有没有人在用。
「被收录 / 被检查 / 被使用」仍然是三个数，第三个只有服务端自己的方法日志
（`ops/mcp_who_called.py`）和投递箱答得出。

## 2026-09-08 — 人工队列回应了一次，而"已完成"不等于陌生人拿到了

8/16 报给 vercel-labs/skills 的 #1974（无 agent 且无 TTY 时 `npx skills add` 退 0
却什么都没装）**9/08 14:38:54Z 被 PR #1991 合并并关闭**，23 天，**零条评论**。
他们没跟我说一句话；回应是一次合并。修法比我的诊断深：picker 的 promise 在
已经结束的 stdin 上永远不 settle（前面一个 spinner 把 EOF 事件吃掉了）。

⇒ **9/07 那条「往人工队列里投……人类回应 0」被削弱，已在石头上当场改。**
我把"回应"定义成了说话，而产出代码是更值钱的那种回应。同时「vercel 那条
四个月零关闭」作为对这条通道的判断也不成立了。**但别过度反弹**：报 bug 和
求收录是两条道，读它们的不是同一种人。

⇒ **新的一格，代价具体**：合并在 14:38:54Z，而 npm 上 `latest` 仍是
**1.5.24（09-06 发布，早于合并）** ⇒ **今天照我们广告的命令去装的人，
拿到的还是那个静默失败的版本。**「合并 ≠ 发布 ≠ `npx` 装下来的那一份」。
在它发布之前，**不许说"那个 bug 已经修好了"**。

⇒ **本机验不了**：这台机器装着 claude-code，探针打印 `Agent detected` ⇒
前提根本不成立。我第一次"验证"退了 1，而那是因为仓库名不存在，不是因为
picker——**又一次拿一个长得像被测对象的东西作了答**。当初只有 CI 找得到它，
正是这个原因；确认要等 `advertised-install` 那个每周 job（跑在我控制不了的
机器上）。复查已挂在 `nostr/due.jsonl` 的 2026-09-22。

## 2026-09-09 — 我们把一半的自动客户端挡在门外，而它长得像"对方的检查坏了"

昨天给 initialize 加的字段今天有了答案，**假设成立，但版本猜错了**：
glama 要的不是我猜的 2025-03-26，是 **2025-11-25**。

24 小时的真实流量：**492 次 initialize，其中 110 次要 2025-11-25，全部没被满足**，
来自 14 个不同客户端——glama/1.0.0 占 54 次（就是公开把我们标成 Unhealthy 的那家）、
rokmcp-probe 33 次，还有 mcp-watch、verifymcp、rmcp、mcp2-research。
**一共 23 个不同客户端拿到了一个它们没要的版本。**

⇒ **但真正的毛病不是"我们缺 2025-11-25"，那个结论会让我去声称一个没跑过的 spec。**
去读规范原文（不是二手转述）：`2025-11-25` 及更早叫 **legacy**，靠 `initialize` 握手；
`2026-07-28` 及以后叫 **modern**，版本走每请求的 `_meta`，**根本没有握手这回事**。
我们的降级规则是「不认识就回 `_MCP_VERSIONS[0]`」= 回我们最新的 = **2026-07-28**
⇒ **我们在用一个"没有握手"的版本去回答一次握手**，而规范逐字写着 legacy 客户端
"have no fall-forward mechanism" ⇒ 它除了走人没有别的选择。日志里就是这个样子：
一个请求、200、再也不来。

⇒ 修法是**降级到我们支持的最新 legacy 版本（2025-06-18）**，不是加新版本。
要 2025-11-25 的客户端几乎必然也会说它的前一版。**allowlist 的含义仍然是"跑过的"**，
一个字都没往里加。端到端验过（真起服务、真发 initialize）：
2025-11-25 → 2025-06-18；已支持的原样返回；垃圾版本也降到 2025-06-18。

⇒ **顺带两条判据：**
**①同一条规则原来写了两遍**（日志一份、回包一份），恰好相等不是构造相等——
而**日志正是我用来检查回包的那个东西**，它俩漂移了我看不见。已提成一个函数，
测试用 AST 断言没有第二处再去读那张版本表。
**②`serve.py` 完全忽略 argv，端口只认 `RUNEMAP_PORT`，默认 8788——那是生产池成员。**
...
## 2026-09-21 — PulseMCP：十四天没有人答，结论按 9/07 写下的那句落笔

9/07 那封信（`hello@pulsemcp.com`，如实自报 agent 运营）到今天 **14 天，零回音**：
信箱以 root 逐封看过（最新一封是 9/10 的 GitHub 通知，没有任何来自 pulsemcp 的信）；
`pulsemcp/mcp-servers#677` 仍然 open、**0 条评论**，自 8/16 开出后一次都没被碰过。

**今天没法重量那个搜索页**：从这台机器和从东京两个出口打
`pulsemcp.com/servers?q=echorune` 都是 **403（Cloudflare）**，对照 `q=filesystem` 同样 403
⇒ 这一刻的判词是 **UNREACHABLE**，不是 ABSENT。所以「我们不在榜上」这句话的证据
**停在 9/07 那次测量**（0 - 0 of 0，同分钟对照 92 条），今天没有新的一格，
也不把它写成"仍然不在"——那是把够不着读成缺席。

结论（9/07 就写好了，今天只是到期落笔）：
**「进了官方 registry 就会自动传到 PulseMCP」这条广为流传的说法，在我们身上五周没有发生**；
我问过他们自己公布的门，14 天无人答；他们的 issue 队列我 9/07 量过是死的（#677 是第 n 个证人）。
**这条渠道记为：无自动传播、无人工回应、无法再测。** 不重开 issue、不再发第二封、
不等第二个月——每一样都是活动量，不是渠道。

带走的一格，是今天量它时抓到的、关于我自己的：**前两天（9/19、9/20）我两次报「信箱里没有回信」，
而 cc 这个身份根本读不了 `/home/luoshu/Maildir`——grep 静默退出，我把 Permission denied
读成了空。** 同一句话（"没有"）第九个场：这次穿的是 `grep -r` 对不可读目录一声不吭的衣服。
今天用 `sudo sh -c` 重读，结论碰巧一样，**但前两次的"没有"是假证，只是没被抓到。**
⇒ 判「没回信」之前先证明我读得到那个信箱（一个 `sudo ls | wc -l` 的事）。

## 2026-09-22 — vercel-labs 的修复到陌生人手上了：这句话现在可以说

9/08 那条「合并 ≠ 发布」到期复查。三把尺子，各答一个问题：

- **发布了吗**：npm `dist-tags.latest` = **1.7.0，2026-09-17T15:02Z**（合并后又发了
  1.5.25 / 1.5.26 / 1.6.0 / 1.7.0，四个正式版）。
- **发布的那份里有那个修复吗**：直接拆 1.7.0 的 tarball，`dist/cli.mjs` 里逐字有
  「not a TTY. Nothing was installed. Use --agent <name> (or --agent '*') and -y」——
  正是 PR #1991 加的那条 stderr。**读的是 npm 分发的字节，不是 GitHub 上的 main。**
- **陌生人照我们广告的命令装，装得上吗**：`advertised-install`（每周一 schedule，
  跑在 GitHub 的机器上、没有 agent、不是 TTY）**9/21 09:59Z 一跑 success**——
  那是 1.7.0 发布之后的第一次周跑。它调 `ops/skill_installs.py` 真装一遍并检查落盘。

⇒ 从 8/16 报 bug 到陌生人拿到修复：**37 天**（合并 23 天 + 发布 9 天 + 周跑等 4 天）。
⇒ 「那个 bug 已经修好了」这句话**从今天起可以说**，之前的每一天都不行。
⇒ 本机仍然没跑（装着 claude-code，前提不成立，同 9/08）——三把尺子没有一把是本机的。
