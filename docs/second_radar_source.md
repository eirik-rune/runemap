...
## 2026-09-04 — ack 只压住了故障那一半，而它们是同一次抖动的两半

自 state ack 装上（08-31）以来，源体检铃 7 条，其中**「KNMI 恢复」5 条 / 4.7 天
= 1.1 次每天，我每次的答案一字不差**（量过、判过、写在这份 doc 里、ack 到 09-12）。

**这不是"铃太吵"，是机制和我记录下的判断拧着**：我 ack 的原话是"这个源每隔几天
进出失败集合一次，我量过 1165 轮 13.2%"，而 ack 只压制**失败与 6h 复述**，
**恢复那一半照响**。于是对一个被判定为慢性抖动的源，我恰好只收到没用的那一半。

**设计已经想清楚，先不实现**（这一段就是将来动手时的起点）：
当**这一轮恢复的源全部**被一条 state ack **精确覆盖**（判词相同、未过期）时，
恢复不响，只写进心跳日志并点名；**只要有一个没被覆盖，照响、照点全部的名**。
全部恢复对未 ack 的源仍然是新闻——"好消息也是消息"那条不动。

**为什么今天不做**：8/28 我给这台状态机写死过一条可证伪的线——**实际响铃到
5 次/天才动手**。现在总频率 1.5/天、这个形状 1.1/天，**没到线**。
而石头上紧跟那条的一句是：*当天下午就因为摩擦是真的而推翻自己刚设的线，
那条线以后就不算数了。* 摩擦是真的，那正是这条线存在的理由。
五天里这台状态机已经被我改了三次，每次都在 48 小时内被下一次破坏或暴露。

⇒ **触发条件写死：源体检铃实际频率到 5 次/天，或这个"恢复"形状单独到 3 次/天。**
到了就照上面那段实现，不用重新论证。

## 2026-09-05 — 09-11 那个决定的依据，我一直用错了一个数

装了 `ops/source_availability.py`（从日志现场算，不留在记忆里）。第一次跑出来的表
**推翻了我随身带的那个数字**：

```
knmi-amsterdam   1638 轮   可用 47.3%   NO-MAP 83, NO-READER 16, THROTTLED 638, THROTTLED-STUCK 127
chrzc-zurich     1612 轮   可用 77.7%
redemet-saopaulo 1648 轮   可用 89.8%
dmi-copenhagen   1631 轮   可用 98.2%   ← 今天响铃的那个，最健康的之一
```

**我一直引的是「13.2% 不可用」，而那个数只算 `THROTTLED-STUCK`（卡住成段的 127 轮），
没算普通的 `THROTTLED`（638 轮）。** 两个数都是真的，它们回答不同的问题——
"它卡住过几段"和"我们有多少轮问不到它"——**而我拿窄的那个当宽的用了，
用在一个要决定这个源去留的判断上。** 决定日 09-11 的输入从此是 47.3%。

**这个工具第一次跑就往对我有利的方向错了两次，两次都在同一个地方：**
① 判词正则是一张**封闭白名单**，安静地丢掉了 `THROTTLED-STUCK` 的 127 轮（全是坏轮），
也丢掉了 smhi 的 `STALE` 52 轮——**于是 smhi 显示 100.0%**。
*匹配不到任何东西又不出声的白名单是 fail-open*，这条我为备份脚本的 KEEP 名单写过，
**在一个专门用来防止我信任记忆数字的工具里，一小时内又犯了一次。**
② 改成"任意大写词"之后要求至少 3 个字符，而 **`OK` 只有两个** ⇒ 所有成功轮次不匹配，
人人 0%。⇒ 现在第二组被钉成源名的形状（小写开头），换行的堆栈不会被读成源名；
**没预料到的判词照样计入分母，并在输出里点名喊出来**，不许静默丢弃。

### 09-05 — 决定的三个输入齐了，而它们把这个二选一变成了三选一

**① 可用率 47.3%**（1638 轮，08-13 起）。见上一节：我原来引的 13.2% 只算 THROTTLED-STUCK。

**② 荷兰读者：0。** 6.9 天、12628 次非扫描请求，`amsterdam|rotterdam|utrecht|
eindhoven|hague|haag|groningen|tilburg|almere|breda|nijmegen|阿姆斯特丹|鹿特丹|海牙`
**一次都没有**。对照组只命中 `zurich` 80 次——柏林、哥本哈根同样是 0，
所以这不是荷兰特殊，是**整个欧洲几乎没人问**（头部是 tokyo 4924、chiangmai 1072、
princeton 1060、washington 827、LA 576）。
**窗口是量出来的不是我传给它的**：我给 journalctl 传了 `--since 14 days`，
而它实际只保留到 08-29 ⇒ **6.9 天，不是 14 天**。差点报一个大一倍的分母。

**③ 坏的时候读者看到什么：诚实的那一种。**
`radar: fetching -- our last ask got no frames; waiting; weather above is live`。
不是"空网格和晴天同形"那种沉默的坏 —— 天气正文照常、雷达明说自己没有。
⇒ **它失败的代价对读者接近零。**

**⇒ 于是真正的成本不在我们这侧，而我 8/20 把问题框错了。**
KNMI 那把 key 是**所有未注册用户共用**的。我们每 20 分钟去问一次，
**为一个 6.9 天里零人问过的国家，花的是陌生人的配额。**
石头上那条写的是"我的探针花的不是我的预算，是陌生人的"——
它一直被我当成一条测量纪律，而这里它是**决定本身**。

**所以选项是三个不是两个：**
①接受共享 key 并写进文档（现状）· ②砍掉荷兰 · ③**保留源，停掉后台轮询，
只在真的有人问荷兰城市时现取**。零读者 ⇒ 零共享配额消耗；有人问 ⇒ 他拿到实时的一帧。
**③ 是我一直没想到的那个**，因为我把"这个源对我们够不够好"当成了问题，
而问题是"我们在不在为零收益占别人的池子"。

09-11 之前落实哪一个，还没定；但依据从此是上面三个数，不是我记得的那个 13.2%。

### 09-05 补：消耗共享配额的不是服务，是我的探针

追下去发现前一节把"我们"写笼统了。**只有 `render_scene.py` 引用 KNMI ⇒ 服务本身
已经是「有人问才取」**，而荷兰 6.9 天零人问 ⇒ **读者驱动的消耗约等于 0。**

那 1638 轮是谁问的？**`/etc/cron.d/runemap-sources`，每 20 分钟一轮的体检探针**
（3 请求/周期 × 72 轮 = **216 次/天**，代码里逐字写着 "shared by every reader"）。
⇒ **我的探针是那把共享 key 的全部消耗者**，为一个没有读者的国家。

这是 8/03 那条的最锐版本：*探针的询问就是真用户的询问，我的测量会替真人改变系统状态*
——只不过这里探针是**唯一**的用户，而账记在陌生人头上。
⇒ 所以选项 ③ 不是"改服务"，是**改我自己的监控**：这件事从头到尾在我这一侧。

**而代码拦住了我本来会犯的错。** 最省事的做法是加第二条 cron 只跑 knmi、拉长到 6 小时——
`source_health.py` 里写着：**子集运行会把它没看到的源的连胜标签清零**，
于是"只调试一个源"会静默重置所有源的升级状态。**一个我没读就会踩的坑，而它写在那儿。**

**要做的改动（9/11 前，专门做，不周六晚上顺手改生产监控）：**
在 `source_health.py` 里支持**每源探测间隔**，被间隔跳过的源报成 `SKIPPED` 而**不是**
清零它的连胜、也**不**计入可用率分母（否则 `ops/source_availability.py` 会把"我没问"
读成"它没坏"——空结果那一族的又一个场）。KNMI 设 6 小时 ⇒ **216 次/天 → 12 次/天，降 94%**，
而可用率仍然量得到，只是分辨率变粗，且这件事要印在输出里。

**不采纳的两个更省事的方案，理由写下来免得以后又想起来**：
①把整队探测频率一起降 —— 为一个源赔上所有源的可见度；
②让 KNMI 的体检读自己的缓存 —— 那样它报的是缓存不是上游，**替身仪器**，
正是这个仓库记了一整年的病。

## 9/06：改完了，而写的时候有三件是代码和测试替我发现的

`PROBE_EVERY` 落地（PR #228，835 测试绿，四条消融各能把新测试打红）。
KNMI 6 小时一次 ⇒ **216 次/天 → 4 次/天**（比计划的 12 更少：一轮问一次，不是三次）。
`SKIPPED` 不进连胜、不进分母、命名即照问。

**三件我没想到的，逐条：**

**① 同一个词有三个读者，而我只改了一个。** `source_health.py` 印 `SKIPPED`，
`heartbeat.sh` 的铃数的是 `^(OK|THROTTLED)` 的**补集** ⇒ 不改它，这口铃会为一个
**我自己排的沉默**每 20 分钟响一次，永远。`ops/source_availability.py` 会把
"没问"算进分母，让 KNMI 显得比实测更坏。⇒ **一个新判词要去找它的所有读者，
而它们住在不同的仓库里** —— 改一个不会让另一个跟着改。三处都点过火
（旧正则确实响、新正则安静、真故障照响）。

**② 两个已有的测试用的正是这个 label，而它们读的是 `/tmp` 里生产留下的文件。**
它们当场变红、打印 `SKIPPED`。⇒ 如果它们当初写得含糊一点（用 fake-city），
**唯一一个有间隔的源，它的升级逻辑会因为一个共享文件的巧合而无人测试**，
而且是绿的。同族于「测试不许读机器上碰巧有什么」，这次碰巧有的是我今天刚造的东西。

**③ 我自己的新测试里，状态在用例之间泄漏。** 同一个 pid 一个 tmp 目录、setUp 不清，
于是一个用例留下的时间戳让下一个用例的**第一次运行**就跳过 —— 一条断言"第一次运行"的
测试其实在量第二次。**捂热的缓存和不清的状态是同一个形状。**

**决定（9/11 那条到期项的答案，提前给出）：留着荷兰源，接受共享配额，并把
探针从消耗者的位置上撤下来。** 注册 key 8/20 问过、8/28 追问过、至今无回音 ——
沉默不是同意也不是拒绝，但它不再是我排期的理由。可用率仍然量得到（45.2%，
窗口 8/13–9/06），只是分辨率变粗，而**那个数字从此描述的是这个源，不再是我的探针**。

## CNPJ / 巴西实体这一问：到期未答，决定写在这里（2026-09-16）

**经过**：8/28 拿到 REDEMET (SDOP) 具名、有条件、可撤销的授权，六条条件全部落地
（含我主动更正的请求量：原信写 10 分钟一次，实测约 18 次/周期，**错了 18 倍且方向对我有利**，
不查永远不会有人发现）。9/02 我就"我们没有 CNPJ、没有巴西实体"这一点主动去问，
**他们没有回答，而是转办**：「今后一切事宜走 ximenesjx@decea.mil.br」。同一小时我把同一个问题
以一页纸的形式重发到那个地址（投递成功）。**到今天 9/16，14 天，零回音。**

**决定：继续按现有授权和它的六条条件服务巴西，不扩大用途，不主动再催。**
理由：授权本身是具名的、有日期的、条件明确且已全部满足；实体问题是**我方主动提出**的，
不是他们设的门槛。**沉默不是同意，也不是撤销** —— 所以它不改变授权的效力，
也不许被读成"他们默许了我们没有实体"。

**这一条的作用域写死**：如果将来他们提出实体要求、或撤销授权，立刻停用并在这里记一笔。
**不再为这件事排期、不再等**——继续围着一段沉默做计划就是 KNMI 那个陷阱
（一个只能由对方开口才能解除的条件，会让我无限期挂着）。
