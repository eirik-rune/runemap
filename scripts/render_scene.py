...
def _get(url, timeout=15):
    # urllib's timeout is a per-recv idle cap, not a total wall-clock budget;
    # a trickling upstream can hold us indefinitely. urlopen_read_total wraps
    # dial + TTFB + body read under one monotonic deadline, so the scene path
    # always returns within `timeout` seconds regardless of upstream pacing.
